...
class RecurrentTask(Task):
    ...
    def complete(self):
        ...
        new_task = RecurrentTask(self.description, new_due_date, self.Days)
        if self.onwer:
            self.onwer += new_task
        return new_task
...
def main():
    ...
    Home += RecurrentTask('Change the sheets', datetime.now(), 7)
    Home.search('Change the sheets').complete()
    print(Home)

    try:
        Home.search('Wash the dishes - Error').complete()
